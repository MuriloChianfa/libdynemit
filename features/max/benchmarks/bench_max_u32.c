...
{
    bench_opts_t opts;
    int rc = bench_parse_opts(argc, argv, &opts, "Max (u32)");
    if (rc != 0) return rc < 0 ? 1 : 0;

    simd_level_t lvl = opts.force_level_set ? opts.force_level : detect_simd_level();
    max_u32_fn_t func = max_u32_select(lvl);

    if (opts.auto_detect && bench_auto_detect_open("max_u32", lvl) != 0) return 1;
    if (opts.csv) bench_csv_header();
    else          bench_print_header("Max (u32)", lvl, opts.force_level_set);
