...
static void
bench_size(size_t n, int csv, simd_level_t lvl, hill_estimator_f64_fn_t func)
{
...
{
    bench_opts_t opts;
    int rc = bench_parse_opts(argc, argv, &opts, "Hill (f64)");
    if (rc != 0) return rc < 0 ? 1 : 0;
    simd_level_t lvl = opts.force_level_set ? opts.force_level : detect_simd_level();
    hill_estimator_f64_fn_t func = hill_estimator_f64_select(lvl);
    if (opts.auto_detect && bench_auto_detect_open("hill_f64", lvl) != 0) return 1;
    if (opts.csv) bench_csv_header();
    else          bench_print_header("Hill (f64)", lvl, opts.force_level_set);
