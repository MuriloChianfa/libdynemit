...
static void
bench_size(size_t n, int csv, simd_level_t lvl, histogram_u16_fn_t func)
{
    ...
    uint16_t bounds[] = {16384, 32768, 49152};
    uint64_t out[4];
    for (int w = 0; w < 10; w++) func(data, n, bounds, 3, out);
    double times[BENCH_TRIALS];
    for (int t = 0; t < trials; t++) {
        double t0 = bench_now_sec();
        for (int i = 0; i < iters; i++) func(data, n, bounds, 3, out);
...
{
    bench_opts_t opts;
    int rc = bench_parse_opts(argc, argv, &opts, "Histogram (u16)");
    if (rc != 0) return rc < 0 ? 1 : 0;
    simd_level_t lvl = opts.force_level_set ? opts.force_level : detect_simd_level();
    histogram_u16_fn_t func = histogram_u16_select(lvl);
    if (opts.auto_detect && bench_auto_detect_open("histogram_u16", lvl) != 0) return 1;
    if (opts.csv) bench_csv_header();
    else          bench_print_header("Histogram (u16)", lvl, opts.force_level_set);
