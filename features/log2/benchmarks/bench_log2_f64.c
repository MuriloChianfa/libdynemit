...
static void
bench_size(size_t n, int csv, simd_level_t lvl, log2_f64_fn_t func)
{
    ...
    for (size_t i = 0; i < n; i++)
        in[i] = 1.0 + (double)i;
    for (int w = 0; w < 10; w++) func(in, out, n);
...
{
    bench_opts_t opts;
    int rc = bench_parse_opts(argc, argv, &opts, "Log2 (f64)");
    if (rc != 0) return rc < 0 ? 1 : 0;

    simd_level_t lvl = opts.force_level_set ? opts.force_level : detect_simd_level();
    log2_f64_fn_t func = log2_f64_select(lvl);

    if (opts.auto_detect && bench_auto_detect_open("log2_f64", lvl) != 0) return 1;
    if (opts.csv) bench_csv_header();
    else          bench_print_header("Log2 (f64)", lvl, opts.force_level_set);
