...
static void
mul_f32_sse2(const float *a, const float *b, float *out, size_t n)
{
    size_t i = 0;
    const size_t step = 4;
    for (; i + step <= n; i += step) {
...
static void
mul_f32_sse42(const float *a, const float *b, float *out, size_t n)
{
    size_t i = 0;
    const size_t step = 4;
    for (; i + step <= n; i += step) {
...
static void
mul_f32_avx(const float *a, const float *b, float *out, size_t n)
{
    size_t i = 0;
    const size_t step = 8;
    for (; i + step <= n; i += step) {
...
static void
mul_f32_avx2(const float *a, const float *b, float *out, size_t n)
{
    size_t i = 0;
    const size_t step = 8;
    for (; i + step <= n; i += step) {
...
static void
mul_f32_avx512f(const float *a, const float *b, float *out, size_t n)
{
    size_t i = 0;
...
static void
mul_f32_neon(const float *a, const float *b, float *out, size_t n)
{
    size_t i = 0;
    for (; i + 4 <= n; i += 4) {
...
static void
mul_f32_sve(const float *a, const float *b, float *out, size_t n)
{
    uint64_t i = 0;
...
static void
mul_f32_sve2(const float *a, const float *b, float *out, size_t n)
{
    uint64_t i = 0;
    uint64_t vl = svcntw();
    ...
    for (; i + 2 * vl <= n; i += 2 * vl) {
        ...
        svst1_f32(ptrue, out + i, svmul_f32_x(ptrue, va0, vb0));
        svst1_f32(ptrue, out + i + vl, svmul_f32_x(ptrue, va1, vb1));
    }
    svbool_t pg = svwhilelt_b32(i, (uint64_t)n);
...
mul_f32_fn_t
mul_f32_select(simd_level_t level)
{
    switch (level) {
#if defined(__x86_64__) || defined(__i386__)
    ...
    case SIMD_SSE4_2:  return mul_f32_sse42;
    case SIMD_SSE2:    return mul_f32_sse2;
#endif
#ifdef __aarch64__
    case SIMD_SVE2:    return mul_f32_sve2;
    case SIMD_SVE:     return mul_f32_sve;
    case SIMD_NEON:    return mul_f32_neon;
#endif
    case SIMD_SCALAR:
    default:           return mul_f32_scalar;
}
}
