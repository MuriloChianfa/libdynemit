...
static void
bench_size(size_t n, int csv, simd_level_t lvl, mul_f32_fn_t func)
{
    ...
    if (!a || !b || !out) { free(a); free(b); free(out); return; }

    for (size_t i = 0; i < n; i++) {
        a[i] = (float)i * 0.5f;
        b[i] = (float)i * 0.25f + 1.0f;
    }
    for (int w = 0; w < 10; w++) func(a, b, out, n);

    double times[BENCH_TRIALS];
    for (int t = 0; t < trials; t++) {
        double t0 = bench_now_sec();
        for (int i = 0; i < iters; i++) func(a, b, out, n);
...
{
    bench_opts_t opts;
    int rc = bench_parse_opts(argc, argv, &opts, "Mul (f32)");
    if (rc != 0) return rc < 0 ? 1 : 0;

    simd_level_t lvl = opts.force_level_set ? opts.force_level : detect_simd_level();
    mul_f32_fn_t func = mul_f32_select(lvl);

    if (opts.auto_detect && bench_auto_detect_open("mul_f32", lvl) != 0) return 1;
    if (opts.csv) bench_csv_header();
    else          bench_print_header("Mul (f32)", lvl, opts.force_level_set);
