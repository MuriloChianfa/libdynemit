...
static void
bench_size(size_t n, int csv, simd_level_t lvl, hhi_u16_fn_t func)
{
...
{
    bench_opts_t opts;
    int rc = bench_parse_opts(argc, argv, &opts, "HHI (u16)");
    if (rc != 0) return rc < 0 ? 1 : 0;

    simd_level_t lvl = opts.force_level_set ? opts.force_level : detect_simd_level();
    hhi_u16_fn_t func = hhi_u16_select(lvl);

    if (opts.auto_detect && bench_auto_detect_open("hhi_u16", lvl) != 0) return 1;
    if (opts.csv) bench_csv_header();
    else          bench_print_header("HHI (u16)", lvl, opts.force_level_set);
