...
static double
hhi_histogram_sse42(const uint64_t *counts, size_t n)
{
    return hhi_histogram_sse2(counts, n);
}
...
static double
hhi_histogram_avx2(const uint64_t *counts, size_t n)
{
    return hhi_histogram_avx(counts, n);
}
...
{
    return hhi_histogram_scalar(counts, n);
}

__attribute__((target("+sve")))
static double
hhi_histogram_sve(const uint64_t *counts, size_t n)
{
    return hhi_histogram_scalar(counts, n);
}
...
static double
hhi_histogram_sve2(const uint64_t *counts, size_t n)
{
    return hhi_histogram_scalar(counts, n);
}
...
hhi_histogram_fn_t
hhi_histogram_select(simd_level_t level)
{
    switch (level) {
#if defined(__x86_64__) || defined(__i386__)
    case SIMD_AVX512_VBMI2:
    case SIMD_AVX512F: return hhi_histogram_avx512f;
    case SIMD_AVX2:    return hhi_histogram_avx2;
    case SIMD_AVX:     return hhi_histogram_avx;
...
DYNEMIT_IFUNC_SETUP(hhi_histogram_fn_t, hhi_histogram, hhi_histogram_resolver)

#if defined(DYNEMIT_NO_IFUNC)
double hhi_histogram(const uint64_t *counts, size_t n)
{
    return DYNEMIT_IFUNC_INVOKE(hhi_histogram, (counts, n));
}
#else
#if defined(__x86_64__) || defined(__i386__)
...
#endif
double hhi_histogram(const uint64_t *counts, size_t n)
    DYNEMIT_IFUNC_ATTR("hhi_histogram_resolver");
#endif
