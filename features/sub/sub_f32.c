...
static void
sub_f32_sse2(const float *a, const float *b, float *out, size_t n)
{
    size_t i = 0;
    const size_t step = 4;
    for (; i + step <= n; i += step) {
...
static void
sub_f32_sse42(const float *a, const float *b, float *out, size_t n)
{
    size_t i = 0;
    const size_t step = 4;
    for (; i + step <= n; i += step) {
...
static void
sub_f32_avx(const float *a, const float *b, float *out, size_t n)
{
    size_t i = 0;
    const size_t step = 8;
    for (; i + step <= n; i += step) {
...
static void
sub_f32_avx2(const float *a, const float *b, float *out, size_t n)
{
    size_t i = 0;
    const size_t step = 8;
    for (; i + step <= n; i += step) {
...
static void
sub_f32_avx512f(const float *a, const float *b, float *out, size_t n)
{
    size_t i = 0;
...
static void
sub_f32_sve(const float *a, const float *b, float *out, size_t n)
{
    uint64_t i = 0;
    ...
    do {
        ...
        svst1_f32(pg, out + i, svsub_f32_x(pg, va, vb));
        i += svcntw();
...
static void
sub_f32_sve2(const float *a, const float *b, float *out, size_t n)
{
    uint64_t i = 0;
    uint64_t vl = svcntw();
    ...
    for (; i + 2 * vl <= n; i += 2 * vl) {
        ...
        svst1_f32(ptrue, out + i, svsub_f32_x(ptrue, va0, vb0));
        svst1_f32(ptrue, out + i + vl, svsub_f32_x(ptrue, va1, vb1));
    }
    svbool_t pg = svwhilelt_b32(i, (uint64_t)n);
...
sub_f32_fn_t
sub_f32_select(simd_level_t level)
{
    switch (level) {
#if defined(__x86_64__) || defined(__i386__)
    case SIMD_AVX512_VBMI2:
    case SIMD_AVX512F: return sub_f32_avx512f;
    case SIMD_AVX2:    return sub_f32_avx2;
    case SIMD_AVX:     return sub_f32_avx;
...
DYNEMIT_IFUNC_SETUP(sub_f32_fn_t, sub_f32, sub_f32_resolver)

#if defined(DYNEMIT_NO_IFUNC)
void sub_f32(const float *a, const float *b, float *out, size_t n)
{
    DYNEMIT_IFUNC_INVOKE(sub_f32, (a, b, out, n));
}
#else
#if defined(__x86_64__) || defined(__i386__)
...
#endif
void sub_f32(const float *a, const float *b, float *out, size_t n)
    DYNEMIT_IFUNC_ATTR("sub_f32_resolver");
#endif
