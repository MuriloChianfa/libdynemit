...
static void
bench_size(size_t n, int csv, simd_level_t lvl, entropy_histogram_fn_t func)
{
    const int trials = BENCH_TRIALS;
    int iters = bench_iters_for_size(n);
    uint64_t *counts = aligned_alloc(64, n * sizeof(uint64_t));
    if (!counts) return;

    for (size_t i = 0; i < n; i++)
        counts[i] = (uint64_t)((i % 127) + 1);
    for (int w = 0; w < 10; w++) func(counts, n);

    double times[BENCH_TRIALS];
    for (int t = 0; t < trials; t++) {
        double t0 = bench_now_sec();
        for (int i = 0; i < iters; i++) func(counts, n);
...
{
    bench_opts_t opts;
    int rc = bench_parse_opts(argc, argv, &opts, "Entropy (histogram)");
    if (rc != 0) return rc < 0 ? 1 : 0;

    simd_level_t lvl = opts.force_level_set ? opts.force_level : detect_simd_level();
    entropy_histogram_fn_t func = entropy_histogram_select(lvl);

    if (opts.auto_detect && bench_auto_detect_open("entropy_histogram", lvl) != 0) return 1;
    if (opts.csv) bench_csv_header();
    else          bench_print_header("Entropy (histogram)", lvl, opts.force_level_set);
