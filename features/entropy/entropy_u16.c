...
entropy_u16_sse2(const uint16_t *data, size_t n)
{
    ...
    /* in-place compaction: pack non-zero counts to front */
    size_t ndirty = 0;
    for (size_t i = 0; i < 65536; i++)
        if (hist[i] != 0)
            hist[ndirty++] = hist[i];
...
entropy_u16_avx512f(const uint16_t *data, size_t n)
{
    ...
    /* pack non-zero counts using compress-store */
    size_t ndirty = 0;
    __m512i vzero = _mm512_setzero_si512();
    for (size_t j = 0; j < 65536; j += 16) {
        __m512i v  = _mm512_loadu_si512(hist + j);
