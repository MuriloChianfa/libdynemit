...
static void
bench_size(size_t n, int csv, simd_level_t lvl, radixs_u32_fn_t func)
{
    const int trials = BENCH_TRIALS;
    int iters = bench_iters_for_size(n);
    uint32_t *in  = aligned_alloc(64, n * sizeof(uint32_t));
    uint32_t *out = aligned_alloc(64, n * sizeof(uint32_t));
...
{
    bench_opts_t opts;
    int rc = bench_parse_opts(argc, argv, &opts, "RadixSort (u32)");
    if (rc != 0) return rc < 0 ? 1 : 0;

    simd_level_t lvl = opts.force_level_set ? opts.force_level : detect_simd_level();
    radixs_u32_fn_t func = radixs_u32_select(lvl);

    if (opts.auto_detect && bench_auto_detect_open("radixs_u32", lvl) != 0) return 1;
    if (opts.csv) bench_csv_header();
    else          bench_print_header("RadixSort (u32)", lvl, opts.force_level_set);
