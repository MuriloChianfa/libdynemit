...
static void
bench_size(size_t n, int csv, simd_level_t lvl, radixs_u16_fn_t func)
{
    const int trials = BENCH_TRIALS;
    int iters = bench_iters_for_size(n);
    uint16_t *in  = aligned_alloc(64, n * sizeof(uint16_t));
    uint16_t *out = aligned_alloc(64, n * sizeof(uint16_t));
    if (!in || !out) { free(in); free(out); return; }

    /* Pseudo-random u16 stream stresses the bucket-emit phase across many buckets. */
    uint64_t s = 0xC0FFEEULL ^ (uint64_t)n;
    for (size_t i = 0; i < n; i++) {
        s ^= s << 13; s ^= s >> 7; s ^= s << 17;
        in[i] = (uint16_t)(s & 0xffff);
    }
...
{
    bench_opts_t opts;
    int rc = bench_parse_opts(argc, argv, &opts, "RadixSort (u16)");
    if (rc != 0) return rc < 0 ? 1 : 0;

    simd_level_t lvl = opts.force_level_set ? opts.force_level : detect_simd_level();
    radixs_u16_fn_t func = radixs_u16_select(lvl);

    if (opts.auto_detect && bench_auto_detect_open("radixs_u16", lvl) != 0) return 1;
    if (opts.csv) bench_csv_header();
    else          bench_print_header("RadixSort (u16)", lvl, opts.force_level_set);
