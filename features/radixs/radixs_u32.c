                                ...
                                uint32_t hist[RADIXS_U32_PASSES][RADIXS_U32_BUCKETS])
{
    ...
    size_t i = 0;
    alignas(32) uint32_t b0[8];
    alignas(32) uint32_t b1[8];
    alignas(32) uint32_t b2[8];
    alignas(32) uint32_t b3[8];
    ...
    for (; i + 16 <= n; i += 16) {
        ...
        if (unique == 0xffff) {
            ...
            __m512i offsets = _mm512_set_epi32(
                (int)hist[buf_b[15]], (int)hist[buf_b[14]], (int)hist[buf_b[13]], (int)hist[buf_b[12]],
                (int)hist[buf_b[11]], (int)hist[buf_b[10]], (int)hist[buf_b[ 9]], (int)hist[buf_b[ 8]],
                (int)hist[buf_b[ 7]], (int)hist[buf_b[ 6]], (int)hist[buf_b[ 5]], (int)hist[buf_b[ 4]],
                (int)hist[buf_b[ 3]], (int)hist[buf_b[ 2]], (int)hist[buf_b[ 1]], (int)hist[buf_b[ 0]]);
                     ...
                     uint32_t *hist, int byte_idx)
{
    ...
    for (int k = 0; k < 16; k++) {
        idx_bytes[(size_t)k * 4U] = (uint8_t)((k * 4) + byte_idx);
    }
...
DYNEMIT_IFUNC_SETUP(radixs_u32_fn_t, radixs_u32, radixs_u32_resolver)

#if defined(DYNEMIT_NO_IFUNC)
void radixs_u32(const uint32_t *in, uint32_t *out, size_t n)
{
    DYNEMIT_IFUNC_INVOKE(radixs_u32, (in, out, n));
}
#else
#if defined(__x86_64__) || defined(__i386__)
...
#endif
void radixs_u32(const uint32_t *in, uint32_t *out, size_t n)
    DYNEMIT_IFUNC_ATTR("radixs_u32_resolver");
#endif
