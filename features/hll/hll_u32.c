...
EXPLICIT_RUNTIME_RESOLVER(hll_u32_resolver, hll_u32_fn_t)
{
    return hll_u32_select(detect_simd_level_ts());
}
DYNEMIT_IFUNC_SETUP(hll_u32_fn_t, hll_u32, hll_u32_resolver)

#if defined(DYNEMIT_NO_IFUNC)
double hll_u32(const uint32_t *data, size_t n)
{
    return DYNEMIT_IFUNC_INVOKE(hll_u32, (data, n));
}
#else
#if defined(__x86_64__) || defined(__i386__)
...
#endif
double hll_u32(const uint32_t *data, size_t n)
    DYNEMIT_IFUNC_ATTR("hll_u32_resolver");
#endif
