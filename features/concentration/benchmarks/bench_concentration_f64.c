...
static void
bench_size(size_t n, int csv, simd_level_t lvl, concentration_f64_fn_t func)
{
    ...
    size_t ks[] = {1, 5, 10};
    double ratios[3];
    concentration_result_t result = { .topk_ratios = ratios };
    for (int w = 0; w < 10; w++) func(data, n, total, ks, 3, &result);
    double times[BENCH_TRIALS];
    for (int t = 0; t < trials; t++) {
        double t0 = bench_now_sec();
        for (int i = 0; i < iters; i++) func(data, n, total, ks, 3, &result);
...
{
    bench_opts_t opts;
    int rc = bench_parse_opts(argc, argv, &opts, "Concentration (f64)");
    if (rc != 0) return rc < 0 ? 1 : 0;
    simd_level_t lvl = opts.force_level_set ? opts.force_level : detect_simd_level();
    concentration_f64_fn_t func = concentration_f64_select(lvl);
    if (opts.auto_detect && bench_auto_detect_open("concentration_f64", lvl) != 0) return 1;
    if (opts.csv) bench_csv_header();
    else          bench_print_header("Concentration (f64)", lvl, opts.force_level_set);
